...
{
    public $id,$type,$name,$desc,$image,$opts,$list,$ours,$pid,$stock,$sale;

    public function __construct($id,$name,$desc,$type,$image,$opts,$list,$ours,$pid,$stock,$sale=null) {
        $this->id = $id;
        $this->name=$name;
        $this->desc=$desc;
        $this->image=$image;
    ...
    public function display($type)
    {
        if($type == $this->type || $this->id==$type):?>
            <div class="row beige border border-dark border-right-0 border-left-0 pb-2">
                ...
                <?php if(isset($this->sale)):?>
                <p class="red">
                    Priced to sell! <?=$this->sale;?>% off!
                </p>
                <?php endif; ?>
            </div>
        </div>
